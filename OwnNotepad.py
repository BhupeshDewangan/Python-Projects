...
def opening_file():
    file_location = askopenfilenames(
        filetypes = [("Text Files", "*.txt"), ("All Files", "*.*")])
...
frame_buttom = tk.Frame(root, relief = tk.RAISED, bd=3)
frame_buttom.grid(row=0, column= 0, sticky="ns")


button_open = tk.Button(frame_buttom, text= "OPEN FILE", command = opening_file)
button_open.grid(row= 0, column= 0, padx= 5, pady= 5 )

button_save = tk.Button(frame_buttom, text= "SAVE AS", command= saveing_file)
